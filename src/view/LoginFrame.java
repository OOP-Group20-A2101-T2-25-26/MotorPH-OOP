    ...
    private void authenticateUser() {
        ...
        try (BufferedReader br = new BufferedReader(new FileReader(csvFile))) {
            ...
            while ((line = br.readLine()) != null) {
                ...
                if (user.length == 3 && user[0].equals(username) && user[1].equals(password) && user[2].equals("0")) {
                    loggedIn = true;
                    isAdmin = true;
                    break;
                }
                else if (user.length == 3 && user[0].equals(username) && user[1].equals(password) && user[2].equals("1")) {
                    loggedIn = true;
                    isAdmin = false;
                    break;
                }
        ...
        if (loggedIn && isAdmin) {
            ...
            // For now, we will show a popup message upon successful admin login
            ...
            // Proceed to the main application
            // For now, we will show a popup message upon successful regular login
            // In the next step, we'll open the main app window
            // Proceed to the main application
            dispose(); // Close the login window
            SwingUtilities.invokeLater(() -> {
